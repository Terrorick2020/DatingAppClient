...
import type { EditVideoData, VideoItemWithPsych } from '@/types/videos.types'
...
export const getProfileByIdAsync = createAsyncThunk(
	...
	async (
		data: DataGetProfileByIdAsync,
		{ dispatch }
	): Promise<AsyncThunkRes<TargetProfile>> => {
		try {
			dispatch(setLoad(true))
			...
			switch (data.type) {
				case EProfileRoles.User:
					const [userRes, cmplRes]: [
						...
						api.get(
							`${COMPLS_ENDPOINT}?telegramId=${data.id}&type=received&status=PENDING`
						),
					])
				...
				case EProfileRoles.Psych:
					...
					const response: AxiosResponse<FetchResponse<TargetPsychologistRes>> =
						await api.get(url)
					...
					const psychRes: TargetProfile = {
						id: '' + dataRes.id,
						role: EProfileRoles.Psych,
						photos: dataRes.photos.map(item => ({
							id: '' + item.id,
							photo: item.url,
						})),
						name: dataRes.name,
...
export const addPhotoToUserAsync = createAsyncThunk(
	...
	async (
		...
		try {
			...
			const uploadRes: AxiosResponse<FetchResponse<FetchSavePhotoRes>> =
				await api.post(UPLOAD_PHOTO, formData, {
			...
			const savedRes: AxiosResponse<FetchResponse<any>> = await api.patch(
				`${USER_ENDPOINT}/${toUser}`,
				savedData
			)

			if (savedRes.status !== 200 || !savedRes.data.success) return null
	...
	async (
		...
		try {
			...
			if (delComplaint) {
				...
				const uptComplRes: AxiosResponse<FetchResponse<any>>[] =
					await Promise.all(
						targetUser.complaint?.map(item =>
							api.post(COMPLS_UPT_ENDPOINT, {
								...uptComplBaseData,
								complaintId: item.id,
							})
						) || []
			...
			const serchStatRes: AxiosResponse<FetchResponse<any>> =
				await api.patch(url)

			if (serchStatRes.status !== 200 || !serchStatRes.data.success) return null

			if (isDisp) {
				adminState.profilesList = adminState.profilesList.map(item => ({
					...item,
...
export const selectPsychStatusAsync = createAsyncThunk(
	...
	async (
		data: DataSelPS,
		{ dispatch, getState }
	): Promise<AsyncThunkRes<EPsychStatus>> => {
		try {
			...
			if (data.isDisp) {
				...
				const newProfilesList = profilesList.map(item => ({
					...item,
...
export const getTargetVideoInfoAsync = createAsyncThunk(
	...
	async (
		...
		try {
			...
			const result = videosList.find(item => item.id === id)
	...
	async (_, { getState }): Promise<AsyncThunkRes<boolean>> => {
		try {
			...
			const response: AxiosResponse<
				FetchResponse<Omit<VideoItemWithPsych, 'isLiked'>>
...
const adminSlice = createSlice({
	...
	extraReducers: builder => {
		...
		;(builder.addCase(
			selectPsychStatusAsync.fulfilled,
			(state, action: PayloadAction<AsyncThunkRes<EPsychStatus>>) => {
				switch (action.payload) {
					case 'error':
		...
		;(builder.addCase(
			getProfileByIdAsync.fulfilled,
			(state, action: PayloadAction<AsyncThunkRes<TargetProfile>>) => {
				switch (action.payload) {
					case 'error':
		...
		;(builder.addCase(
			serchProfileStatusAsync.fulfilled,
			(state, action: PayloadAction<AsyncThunkRes<EProfileStatus>>) => {
				switch (action.payload) {
					case 'error':
		...
		;(builder.addCase(
			initComplaintListAsync.fulfilled,
			(state, action: PayloadAction<AsyncThunkRes<ComplaintListItem[]>>) => {
				switch (action.payload) {
					case 'error':
		...
		;(builder.addCase(
			deletePsychVideoAsync.fulfilled,
		...
		;(builder.addCase(
			toggleVideoPublishedAsync.fulfilled,
			(_, action: PayloadAction<AsyncThunkRes<boolean>>) => {
				switch (action.payload) {
					case 'error':
