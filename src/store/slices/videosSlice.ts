		...
		try {
			...
			const fetchVideo = async (): Promise<AsyncThunkRes<PsychAddVideoRes>> => {
				try {
					...
					// Сохраняем previewKey в состоянии
					dispatch(
						setTargetPsychVideo({
							key: data.key,
	...
	async (_, { getState }): Promise<AsyncThunkRes<TargetPsychVideo>> => {
		try {
			...
			const data = {
				...
				previewKey: targetPsychVideo.previewKey,
...
export const editPsychVideoAsync = createAsyncThunk(
	...
	async (
		editData: EditVideoData,
		{ getState }
	): Promise<AsyncThunkRes<VideoItem[]>> => {
		try {
			const rootState = getState() as IState
...
export const getSelfPsychVideosAsync = createAsyncThunk(
	...
	async (
		data: InitSliderData,
		{ dispatch, getState }
	): Promise<AsyncThunkRes<SelfPsychVideos>> => {
		try {
			...
			const response: AxiosResponse<FetchResponse<SelfPsychVideos>> =
				await api.get(url)
...
const videosSlice = createSlice({
	...
	extraReducers: builder => {
		...
		builder.addCase(
			psychAddVideoAsync.fulfilled,
			(state, action: PayloadAction<AsyncThunkRes<PsychAddVideoRes>>) => {
		...
		builder.addCase(
			publishPsychVideoAsync.fulfilled,
			(state, action: PayloadAction<AsyncThunkRes<TargetPsychVideo>>) => {
				switch (action.payload) {
					case 'error':
		...
		builder.addCase(
			editPsychVideoAsync.fulfilled,
			(state, action: PayloadAction<AsyncThunkRes<VideoItem[]>>) => {
				switch (action.payload) {
					case 'error':
		...
		builder.addCase(
			getTargetPsychVideoAsync.fulfilled,
			(state, action: PayloadAction<TargetPsychVideo | null>) => {
				switch (action.payload) {
					case null:
						state.targetPsychVideo = targetPsychVideoBase
		...
		builder.addCase(
			getSelfPsychVideosAsync.fulfilled,
			(state, action: PayloadAction<AsyncThunkRes<SelfPsychVideos>>) => {
				switch (action.payload) {
					case 'error':
		...
		builder.addCase(
			getShortsAsync.fulfilled,
			(state, action: PayloadAction<AsyncThunkRes<VideoShortsList>>) => {
				switch (action.payload) {
					case 'error':
		...
		builder.addCase(
			getAdminShorrtsAsync.fulfilled,
			(state, action: PayloadAction<AsyncThunkRes<VideoShortsList>>) => {
				switch (action.payload) {
					case 'error':
		...
		builder.addCase(
			toggleShortsLikeAsync.fulfilled,
